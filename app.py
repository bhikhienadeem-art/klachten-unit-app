        ...
        if klachten:
            for k in klachten:
                ...
                with st.expander(f"Klacht #{k.get('id', '?')} - {onderwerp} ({status})"):
                    st.write(f"**Naam:** {k.get('volledige_naam', 'Anoniem')}")
                    st.write(f"**E-mail:** {k.get('email', 'Geen')}")
                    st.write(f"**Telefoon:** {k.get('telefoon', 'Geen')}")
                    st.write(f"**Omschrijving:** {k.get('omschrijving', 'Geen')}")

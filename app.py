...
with tab1:
    ...
    for k in klachten:
        with st.expander(f"Klacht van: {k.get('volledige_naam', 'Anoniem')} - Status: {k.get('status', 'Nieuw')}"):
            ...
            # Email sturen (via mailto link)
            if k.get('email'):
                st.markdown(f'<a href="mailto:{k["email"]}?subject=Update over uw klacht&body=Geachte {k["volledige_naam"]},%0A%0A..." style="text-decoration:none; color:white; background:#004a99; padding:8px 12px; border-radius:5px;">📧 E-mail sturen naar client</a>', unsafe_allow_html=True)
else:
   # --- PUBLIEK FORMULIER ---
